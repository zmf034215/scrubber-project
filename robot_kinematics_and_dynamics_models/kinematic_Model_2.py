...
class Kinematic_Model_ik:
    def __init__(self):

        current_folder = os.getcwd()
        urdf_path = os.path.join(current_folder, "models", "P5_left_arm.urdf")
        ...
        urdf_path = os.path.join(current_folder, "models", "P5_right_arm.urdf")
        ...
        self.solution_left_analysis = np.zeros(7)
        self.solution_right_analysis = np.zeros(7)


    def left_arm_inverse_kinematics(self, cart_interpolation_pose, cart_interpolation_position, current_joint_position):
        # 获取目标位姿
        target_pose = np.eye(4)
        target_pose[:3, :3] = cart_interpolation_pose
        target_pose[:3, 3] = cart_interpolation_position

        # 获取运动学参数
        para = np.zeros(42)
        i = 0
        for joint_id in range(1, 8):
            placement = self.left_arm_pin_model.jointPlacements[joint_id]
            xyz, rpy = placement.translation, pinocchio.rpy.matrixToRpy(placement.rotation)
            start_idx = i * 6
            para[start_idx:start_idx + 3] = xyz  # x,y,z
            para[start_idx + 3:start_idx + 6] = rpy
            i += 1

        # 将目标位姿与运动学参数配准至机械臂基坐标系下
        target_pose_armbase = np.linalg.inv(rpy_pos2h(para[0:6])) @ target_pose
        para_armbase = para.copy()
        para_armbase[0:6] = np.zeros(6, )

        # 获得末端位置
        pos_tcp = np.array([0, 0, 0])

        # 设置限位
        joint_limit = np.array([
            [-2.619, -0.122, -2.478, -2.079, -2.774, -0.9774, -0.4538],
            [2.619,	2.739,	2.478,	0.035,	2.774,	0.9774,	0.4538]
        ])
        theta_analysis = self.solution_left_analysis.copy()
        if np.array_equal(theta_analysis, np.zeros(7)):
            theta_analysis = current_joint_position.copy()
        # 求逆解
        theta_best, theta_analysis, error_info = arm_inverse_kinematics(target_pose_armbase, para_armbase, pos_tcp,
                                                                       joint_limit, current_joint_position,theta_analysis)
        if error_info == 'Reachable':
            self.left_arm_interpolation_result = theta_best
            self.solution_left_analysis  = theta_analysis.copy()
    ...
    def right_arm_inverse_kinematics(self, cart_interpolation_pose, cart_interpolation_position, current_joint_position):
        ...
        i = 0
        for joint_id in range(1, 8):
            placement = self.right_arm_pin_model.jointPlacements[joint_id]
            xyz, rpy = placement.translation, pinocchio.rpy.matrixToRpy(placement.rotation)
            start_idx = i * 6
            para[start_idx:start_idx + 3] = xyz  # x,y,z
            para[start_idx + 3:start_idx + 6] = rpy
        ...
        theta_analysis = self.solution_right_analysis.copy()
        if np.array_equal(theta_analysis, np.zeros(7)):
            theta_analysis = current_joint_position.copy()
        # 求逆解
        theta_best, theta_analysis, error_info = arm_inverse_kinematics(target_pose_armbase, para_armbase, pos_tcp,
                                                                      joint_limit, current_joint_position,theta_analysis)
        if error_info == 'Reachable':
            self.right_arm_interpolation_result = theta_best
            self.solution_right_analysis = theta_analysis.copy()
...
def arm_inverse_kinematics_analytical(target_pose, para, pos_tcp, joint_limit, theta_current,theta_analysis):
    ...
    print(f'theta_dis is{theta_current- theta_analysis}')
