...
class Hybrid_Force_MoveL:
    ...
    def cal_hybrid_force_movel_plan_data(self):
        ...
        # self.left_arm_target_FT_data = target_FT_data[:6]
        self.movel_plan_current_cart_position = deepcopy(self.movel_plan_current_cart.translation)    # 计算左臂当前笛卡尔空间位置
        self.movel_plan_current_cart_pose = deepcopy(self.movel_plan_current_cart.rotation)    # 计算左臂当前笛卡尔空间姿态
        self.movel_plan_current_cart_quat = quaternion.from_rotation_matrix(self.movel_plan_current_cart_pose)    # 计算左臂当前四元数姿态，采用四元数插值
        ...
        self.movel_plan_target_cart_position = deepcopy(self.movel_plan_target_cart.translation)    # 计算左臂目标笛卡尔空间位置
        self.movel_plan_target_cart_pose = deepcopy(self.movel_plan_target_cart.rotation)    # 计算左臂目标笛卡尔空间姿态
        ...
        self.right_arm_movel_plan_current_cart_position = deepcopy(self.right_arm_movel_plan_current_cart.translation)    # 计算左臂当前笛卡尔空间位置
        self.right_arm_movel_plan_current_cart_pose = deepcopy(self.right_arm_movel_plan_current_cart.rotation)    # 计算左臂当前笛卡尔空间姿态

        self.right_arm_movel_plan_current_cart_quat = quaternion.from_rotation_matrix(self.right_arm_movel_plan_current_cart_pose)    # 计算左臂当前四元数姿态，采用四元数插值
        ...
        self.right_arm_movel_plan_target_cart_position = deepcopy(self.right_arm_movel_plan_target_cart.translation)    # 计算左臂目标笛卡尔空间位置
        self.right_arm_movel_plan_target_cart_pose = deepcopy(self.right_arm_movel_plan_target_cart.rotation)    # 计算左臂目标笛卡尔空间姿态
        ...
        if np.linalg.norm(force_direction) < self.MIN_VAL:
            ...
            self.right_arm_movel_plan_target_cart_position = self.right_arm_movel_plan_current_cart_position + self.right_arm_movel_plan_position_delta_disp
            self.right_arm_movel_plan_displacement = np.linalg.norm(self.right_arm_movel_plan_current_cart_position)
    ...
    def hybrid_force_movel_plan_interpolation(self):
        ...
        for interpolation_time in np.arange(0, self.speed_plan.time_length, self.interpolation_period / 1000):
            ...
            # 将导纳部分的位移增量映射到力的方向
                cart_position_increment_2 = self.right_arm_effector_current_speed * (self.interpolation_period / 1000) * np.array(target_FT) / np.linalg.norm(target_FT)
            else: 
                cart_position_increment_2 = np.zeros(3)
